...
fig.read.width <- 12
fig.read.height <- max(ngroups*1.2 +2, 6)
...
qc_read <- instersect(names(figs), c("seq_depth", "aligned_reads", "aligned_pct", "aligned_reads_spikein", "dup_rate", "est_lib_size", "frag_lens_dens"))
if (length(qc_read)){
...
meta %>% 
  ...
  arrange(sample_group, target, sample_replicate) %>% 
  dplyr::select(sample_id, sample_group, target, sample_replicate, bt2_total_reads_target, bt2_total_aligned_target, bt2_overall_alignment_rate_target, bt2_total_aligned_spikein, bt2_overall_alignment_rate_spikein, dedup_percent_duplication, dedup_estimated_library_size) %>% 
  unique.data.frame() %>% 
  DT::datatable(
  class = 'cell-border stripe', 
  rownames = T,  
  ...
  DT::formatRound(columns=c('bt2_overall_alignment_rate_target', 'bt2_overall_alignment_rate_spikein', 'dedup_percent_duplication'), digits=4)
